...
- [ ] T001 Verify Node.js ≥18.x and npm ≥9.x installed (pre-requisite check)
- [ ] T002 Create book/ directory in repository root
- [ ] T003 Initialize Docusaurus 3.x project with classic preset in book/
- [ ] T004 Install core dependencies in book/package.json (Docusaurus 3.x, React 18.x, MDX 3.x)
- [ ] T005 [P] Install development dependencies in book/package.json (TypeScript, markdownlint, pa11y-ci)
- [ ] T006 [P] Configure TypeScript in book/tsconfig.json for Docusaurus config
- [ ] T007 [P] Create directory structure: book/docs/, book/src/, scripts/ in repository root
- [ ] T008 [P] Create static asset directories: book/static/img/, book/static/diagrams/
- [ ] T009 [P] Create .nvmrc file with Node.js 18.x for version consistency
- [ ] T010 [P] Create .editorconfig for consistent code formatting across editors
- [ ] T011 [P] Configure markdownlint rules in book/.markdownlint.json
- [ ] T012 [P] Create .prettierrc in book/ for markdown formatting consistency
- [ ] T013 Create .gitignore with book/build/, book/.docusaurus/, book/node_modules/
...
- [ ] T014 Configure site title and tagline in book/docusaurus.config.ts
- [ ] T015 [P] Configure baseUrl and URL in book/docusaurus.config.ts
- [ ] T016 [P] Configure organizationName and projectName in book/docusaurus.config.ts for GitHub Pages
- [ ] T017 [P] Configure Prism syntax highlighting in book/docusaurus.config.ts (add Python, JavaScript, YAML, Bash, XML)
- [ ] T018 [P] Create basic color variable definitions in book/src/css/custom.css
- [ ] T019 [P] Configure WCAG AA compliant theme colors in book/src/css/custom.css (verify contrast ratios ≥4.5:1)
- [ ] T020 [P] Set up sidebars.js with autogenerated sidebar configuration
- [ ] T021 [P] Add favicon files (favicon.ico, favicon.svg) to book/static/img/
- [ ] T022 [P] Configure favicon and meta tags (viewport, charset) in book/docusaurus.config.ts
- [ ] T023 [P] Create basic homepage component structure in book/src/pages/index.tsx
- [ ] T024 [P] Add content and styling to homepage in book/src/pages/index.tsx
- [ ] T025 [P] Create custom 404 page in book/src/pages/404.tsx
- [ ] T026 [P] Configure navbar in book/docusaurus.config.ts (logo, title, items)
- [ ] T027 [P] Add GitHub repository link to navbar in book/docusaurus.config.ts
- [ ] T028 [P] Configure footer in book/docusaurus.config.ts (copyright, license info, links)
- [ ] T029 Create package.json scripts in book/: start, build, serve
- [ ] T030 [P] Add package.json scripts in book/: lint:md, validate:assets, test:a11y
- [ ] T031 [P] Write per-file size validation (2MB check) in scripts/validate-assets.js
- [ ] T032 [P] Write total folder size validation (100MB check) in scripts/validate-assets.js
- [ ] T033 [P] Configure pa11y-ci accessibility testing in book/package.json and .pa11yci config file

**Checkpoint**: Foundation ready - user story implementation can now begin in parallel

---

## Phase 3: User Story 1 - Local Content Development (Priority: P1) 🎯 MVP

**Goal**: Enable authors to create, edit, and preview textbook content locally with hot reload

**Independent Test**: Clone repo → npm install → npm start → create test.md → see changes in browser within 3 seconds

### Implementation for User Story 1

- [ ] T034 [P] [US1] Create _category_.json for Part 1 (Physical AI) in book/docs/part-01-physical-ai/_category_.json
- [ ] T035 [P] [US1] Create _category_.json for Part 2 (ROS 2) in book/docs/part-02-ros2/_category_.json
- [ ] T036 [P] [US1] Create _category_.json for Part 3 (Simulation) in book/docs/part-03-simulation/_category_.json
- [ ] T037 [P] [US1] Create _category_.json for Part 4 (Isaac) in book/docs/part-04-isaac/_category_.json
- [ ] T038 [P] [US1] Create _category_.json for Part 5 (HRI) in book/docs/part-05-human-robot-interaction/_category_.json
- [ ] T039 [P] [US1] Create _category_.json for Part 6 (Conversational) in book/docs/part-06-conversational-robotics/_category_.json
- [ ] T040 [P] [US1] Create intro.md for Part 1 explaining Physical AI learning outcome in book/docs/part-01-physical-ai/intro.md
- [ ] T041 [P] [US1] Create intro.md for Part 2 explaining ROS 2 learning outcome in book/docs/part-02-ros2/intro.md
- [ ] T042 [P] [US1] Create intro.md for Part 3 explaining Simulation learning outcome in book/docs/part-03-simulation/intro.md
- [ ] T043 [P] [US1] Create intro.md for Part 4 explaining Isaac learning outcome in book/docs/part-04-isaac/intro.md
- [ ] T044 [P] [US1] Create intro.md for Part 5 explaining HRI learning outcome in book/docs/part-05-human-robot-interaction/intro.md
- [ ] T045 [P] [US1] Create intro.md for Part 6 explaining Conversational AI learning outcome in book/docs/part-06-conversational-robotics/intro.md
- [ ] T046 [P] [US1] Create placeholder chapter in Part 1: book/docs/part-01-physical-ai/chapter-01-introduction/index.md
- [ ] T047 [P] [US1] Create placeholder chapter in Part 2: book/docs/part-02-ros2/chapter-01-introduction/index.md
- [ ] T048 [P] [US1] Create placeholder chapter in Part 3: book/docs/part-03-simulation/chapter-01-introduction/index.md
- [ ] T049 [P] [US1] Create placeholder chapter in Part 4: book/docs/part-04-isaac/chapter-01-introduction/index.md
- [ ] T050 [P] [US1] Create placeholder chapter in Part 5: book/docs/part-05-human-robot-interaction/chapter-01-introduction/index.md
- [ ] T051 [P] [US1] Create placeholder chapter in Part 6: book/docs/part-06-conversational-robotics/chapter-01-introduction/index.md
- [ ] T052 [P] [US1] Add 3 sample images (<2MB each) to book/static/img/shared/ for testing
- [ ] T053 [US1] Configure hot reload settings in book/docusaurus.config.ts (optimize for <3s updates)
- [ ] T054 [US1] Test dev server startup time with `npm start` (verify <30s, document if slower)
- [ ] T055 [US1] Create basic CONTRIBUTING.md structure and content guidelines
- [ ] T056 [US1] Add markdown syntax examples and best practices to CONTRIBUTING.md

**Checkpoint**: At this point, authors can run `npm start` and begin creating content with live preview

**Acceptance Validation**:
- ✅ `npm start` launches dev server in <30 seconds
- ✅ Creating new .md file in chapters/ → appears in navigation automatically
- ✅ Editing existing file → browser refreshes in <3 seconds
- ✅ Images display correctly when referenced in markdown
...
**Independent Test**: Push commit to main → GitHub Actions runs → site updates on GitHub Pages within 5 minutes → build failure leaves previous version live
...
**Acceptance Validation**:
- ✅ Push to main → workflow triggers within 1 minute
- ✅ Build completes within 5 minutes
- ✅ Updated content visible on GitHub Pages within 2 minutes of build
- ✅ Build failure → previous deployment remains live (atomic)
- ✅ Feature branch push → no deployment occurs
...
**Independent Test**: Create multi-level content → verify sidebar shows hierarchy → test mobile menu → verify back button navigation
...
**Acceptance Validation**:
- ✅ Sidebar displays hierarchical structure with expandable/collapsible sections
...
- ✅ 3+ levels of navigation hierarchy supported
...
- [ ] T105 Run complete quickstart.md validation end-to-end (fresh clone → running site)
...
1. Complete Phase 1: Setup (T001-T013) → Basic Docusaurus project
2. Complete Phase 2: Foundational (T014-T033) → Configuration ready
3. Complete Phase 3: User Story 1 (T034-T056) → Local development ready
...
1. **Foundation** (Phases 1-2) → Docusaurus configured (33 tasks)
2. **+ US1** (Phase 3) → Local development works (23 tasks) → **Demo to authors**
3. **+ US2** (Phase 4) → Automated deployment works (15 tasks) → **Book goes live**
4. **+ US3** (Phase 5) → Enhanced navigation (13 tasks) → **Full UX complete**
5. **+ Polish** (Phase 6) → Search, SEO, accessibility validated (21 tasks) → **Production ready**
...
**Timeline Estimate**:
- Sequential (1 dev): ~6-8 days (105 tasks × ~20 min avg = ~35 hours)
...
- Phase 3 (US1 - Local Dev): 23 tasks → **MVP**
- Phase 4 (US2 - Deployment): 15 tasks
- Phase 5 (US3 - Navigation): 13 tasks
- Phase 6 (Polish): 21 tasks

**By User Story**:
- User Story 1 (P1): 23 tasks → Local content development
- User Story 2 (P2): 15 tasks → Automated deployment
- User Story 3 (P3): 13 tasks → Navigation & structure
...
**Independent Tests**:
- US1: Install → create content → see in browser (no deployment needed)
- US2: Push to main → see on GitHub Pages (works even without US1 content)
- US3: Navigate site → verify hierarchy (works with basic or rich content)
...
**Atomicity**:
- Split T001 (create dir + initialize) → T002, T003
- Split T007 (3 configs) → T014, T015, T016
- Split T009 (theme) → T018, T019
- Split T011 (homepage) → T023, T024
- Split T015 (validation script) → T031, T032
- Split old T025 (6 chapters) → T046-T051
- Split T029 (CONTRIBUTING.md) → T055, T056
- Split T040 (deployment docs) → T070, T071
- Split T062 (deployment docs) → T094, T095
